...
LPVOID EnableTrampoline(PBYTE Original, PBYTE Detour, SIZE_T Length) {
    LPVOID OldFuncPointer = VirtualAlloc(NULL, Length + 5, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

    CopyMemory(OldFuncPointer, Original, Length);

    uintptr_t OldFuncPointerAddr = ((uintptr_t)Original - (uintptr_t)OldFuncPointer) - 5;
    *(BYTE *)((uintptr_t)OldFuncPointer + Length) = 0xE9;

    *(uintptr_t *) ((uintptr_t)OldFuncPointer + Length + 1) = OldFuncPointerAddr;

    DWORD OldProt;
    VirtualProtect(Original, Length, PAGE_EXECUTE_READWRITE, &OldProt);
    
    uintptr_t Addr = (uintptr_t)(Detour - (uintptr_t)Original) - 5;
    *Original = 0xE9;
    *(uintptr_t *)((uintptr_t)Original + 1) = Addr;
...
DWORD WINAPI mainHakVzlom() {
    ...
    OriginalGethostbyname = (_gethostbyname)EnableTrampoline((PBYTE)&ModuleFuncOffset, (PBYTE)&hijackedgethostbyname, 6);
    ...
    else 
    {
        ...
        if (RegQueryValueExW(hKey, L"MrimDomain", 0, &dwType, (LPBYTE)buf, &dwBufSize) == ERROR_SUCCESS)
        {
            ...
            RegSetValueExW(hKey, L"MrimDomain", 0, REG_SZ, (BYTE*)DEFAULT_DOMAIN, sizeof(DEFAULT_DOMAIN));
            RegSetValueExW(hKey, L"MrimAvatarDomain", 0, REG_SZ, (BYTE*)DEFAULT_AVATAR_DOMAIN, sizeof(DEFAULT_AVATAR_DOMAIN)+1);
            MrimProtocolDomain = (char*)DEFAULT_DOMAIN;
